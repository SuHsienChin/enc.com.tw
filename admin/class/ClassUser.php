...
class ClassUser extends Database
{
    ...
    function setSQL()
    {
        ...
        $this->add_sql = "INSERT INTO $tableName ( name, account, pwd, creatDate, creatOwner, flag) VALUES (";
        $this->add_sql .= " VALUES ('$this->name','$this->account','$this->pwd','$this->creatDate','$this->creatOwner' , ";
        $this->add_sql .= " '$this->flag' ) ";

        $this->delete_sql = "DELETE FROM $tableName  WHERE id='$this->id'";

        $this->update_sql = "UPDATE $tableName SET name='$this->name',account='$this->account',pwd='$this->pwd',creatDate='$this->creatDate',creatOwner='$this->creatOwner',flag='$this->flag' WHERE id='$this->id'";
