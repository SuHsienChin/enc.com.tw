<?php
/**
 * Created by PhpStorm.
 * User: MayumiKing
 * Date: 2016/2/22
 * Time: 下午 12:22
 */

include_once(INCLUDE_DIR . 'db.php');

class Classdlf extends Database
{

    public $db;
    private $id;
    private $cId;
    private $name;
    private $tmp_name;
    private $size;
    private $type;
    private $url;
    private $title;
    private $desc = "";
    private $filename;


    // 類別的建構方法
    public function __construct()
    {
        try {
            $this->db = new PDO("mysql:host=" . DB_HOST . ";dbname=" . DB_DATABASE, DB_USER, DB_PASSWORD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
            /*** echo a message saying we have connected ***/
        } catch (PDOException $e) {
            echo $e->getMessage();
        }
    }

    //取得property 使用magic word
    public function __get($property)
    {
        if (property_exists($this, $property)) {
            return $this->$property;
        }
    }

    //設定property 使用magic word
    public function __set($property, $value)
    {
        if (property_exists($this, $property)) {
            $this->$property = $value;
        }
        return $this;
    }

    // setProperties 方法用來連接表單的欄位以及類別的屬性
    public function setProperties()
    {

        $this->id = $_POST['id'];
        $this->cId = $_POST['cId'];
        $this->name = $_POST['name'];
        $this->tmp_name = $_POST['tmp_name'];
        $this->size = $_POST['size'];
        $this->type = $_POST['type'];
        $this->url = $_POST['url'];
        $this->title = $_POST['title'];
        $this->desc = $_POST['desc'];
        $this->filename = $_POST['filename'];
    }

    // setSQL 方法定義存取資料表所需的各個 SQL 命令
    function setSQL()
    {
        $tableName = "dlfiles";
        $this->select_sql = "SELECT * FROM $tableName ";
        $this->selectClass_sql = "SELECT * FROM $tableName WHERE cId= '$this->cId'";

        $this->add_sql = "INSERT INTO $tableName  ";
        $this->add_sql .= " (cId,name,size,type,url,title,description,filename) ";
        $this->add_sql .= " VALUES ('$this->cId','$this->name','$this->size','$this->type','$this->url','$this->title','$this->desc','$this->filename') ";

        $this->delete_sql = "DELETE FROM $tableName  WHERE id='$this->id'";

        $this->update_sql = "UPDATE $tableName SET cId='$this->cId',name='$this->name',size='$this->size',type='$this->type',url='$this->url',title='$this->title',description='$this->desc',filename='$this->filename'";
        $this->update_sql .= " WHERE id='$this->id'";

        $this->retrieve_sql = "SELECT * FROM $tableName WHERE id='$this->id'";

        $this->countRecordsById_sql = "SELECT count(*) as count FROM $tableName WHERE id='$this->id' ";

    }

} 