...
(function () {
    ...
    function d(k, l) {
        ...
        if (m.indexOf('px') < 0)if (m in n && k.getComputedStyle('border-style') != 'none')m = n[m]; else m = 0;
    ...
    function f(k) {
        ...
        for (var r = 0, s = o.cells.length; r < s; r++) {
            ...
            m += t.$.colSpan || 1;
            var v, w, x, y, z = t.getDocumentPosition().x;
            n ? w = z + d(t, 'left') : v = z + t.$.offsetWidth - d(t, 'right');
            if (u) {
                z = u.getDocumentPosition().x;
                n ? v = z + u.$.offsetWidth - d(u, 'right') : w = z + d(u, 'left');
            } else {
                z = k.getDocumentPosition().x;
                n ? v = z : w = z + k.$.offsetWidth;
            }
            x = Math.max(w - v, 3);
            y = Math.max(Math.round(7 - x / 2), 0);
            l.push({table: k, index: m, x: v, y: q.y, width: x, height: p.$.offsetHeight, padding: y, rtl: n});
        }
        return l;
    };
    function g(k, l) {
        for (var m = 0, n = k.length; m < n; m++) {
            var o = k[m], p = o.padding;
            if (l >= o.x - p && l <= o.x + o.width + p)return o;
        }
        return null;
    };
    function h(k) {
    ...
    function i(k) {
        ...
        function y() {
            ...
            for (var F = 0; F < E; F++) {
                ...
                CKEDITOR.tools.setTimeout(function (J, K, L, M, N, O) {
                    ...
                    if (N)I.setStyle('width', a(N + O * (D ? -1 : 1)));
        ...
        if (!b)m.getDocumentElement().append(n);
        ...
        var C = this.move = function (D) {
            if (!l)return 0;
            var E = l.padding;
            if (!o && (D < l.x - E || D > l.x + l.width + E)) {
                v();
                return 0;
            }
            var F = D - Math.round(n.$.offsetWidth / 2);
            if (o) {
                if (F == t || F == u)return 1;
                F = Math.max(F, t);
                F = Math.min(F, u);
        ...
        requires: ['tabletools'], init: function (k) {
            k.on('contentDom', function () {
                ...
                k.document.getBody().on('mousemove', function (m) {
                    ...
                    if (!n.is('table') && !n.getAscendant('tbody', 1))return;
