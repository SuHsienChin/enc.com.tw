...
(function () {
    ...
    CKEDITOR.dialog.add('templates', function (b) {
        ...
        function d(k, l) {
            ...
            if (k.image && l)n += '<td class="cke_tpl_preview_img"><img src="' + CKEDITOR.getUrl(l + k.image) + '"' + (CKEDITOR.env.ie6Compat ? ' onload="this.width=this.width"' : '') + ' alt="" title=""></td>';
            n += '<td style="white-space:normal;"><span class="cke_tpl_title">' + k.title + '</span><br/>';
            if (k.description)n += '<span>' + k.description + '</span>';
            n += '</td></tr></table>';
        ...
        function f(k) {
            ...
            if (m || g.contains(l)) {
                ...
                if (o) {
                    if (m)p = o.getItem(0); else switch (n) {
                        case 40:
                            p = l.getNext();
                            break;
                        case 38:
        ...
        return {
            ...
            contents: [{
                ...
                elements: [{
                    type: 'vbox',
                    padding: 5,
                    children: [{type: 'html', html: '<span>' + i.selectPromptMsg + '</span>'}, {
                        id: 'templatesList',
                        type: 'html',
                        focus: true,
                        html: '<div class="cke_tpl_list" tabIndex="-1" role="listbox" aria-labelledby="' + h + '">' + '<div class="cke_tpl_loading"><span></span></div>' + '</div>' + '<span class="cke_voice_label" id="' + h + '">' + i.options + '</span>'
                    }, {
                        id: 'chkInsertOpt',
                        type: 'checkbox',
                        label: i.insertOption,
                        'default': j.templates_replaceContent
                    }]
                }]
            }],
            ...
            onShow: function () {
                ...
                CKEDITOR.loadTemplates(j.templates_files, function () {
                    ...
                    if (l.length) {
                        c(g, l);
                        k.focus();
                    } else g.setHtml('<div class="cke_tpl_empty"><span>' + i.emptyListMsg + '</span>' + '</div>');
                });
